...
const nameMapping_1 = __importDefault(require("./nameMapping"));
class VariableRenamer extends modification_1.default {
                ...
                switch (node.type) {
                    ...
                    case 'CatchClause': {
                        if (node.binding && node.binding.type == 'BindingIdentifier') {
                            const variable = new variable_1.Variable(node.binding.name, 'var');
                    ...
                    case 'VariableDeclaration': {
                        for (const declarator of node.declarators) {
                            ...
                            if (declarationScope.variables.has(declarator.binding.name)) {
                                ...
                                if (variable.isBlockScoped() || variable_1.blockScopedTypes.has(node.kind)) {
     ...
     */
    renameVariables(scope, parentMapping = this.nameMapping) {
        const nameMapping = new nameMapping_1.default();
